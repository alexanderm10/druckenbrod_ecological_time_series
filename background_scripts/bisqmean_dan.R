bisqmean_dan = function(y){
  ...
  if(any(is.na(y))){
    stop('y not permitted to have NaNs');
  }
  ...
  for(i in 1:nits){  # iterate max of nits times
    ...
    L1 <- which(abs(u)>=1,arr.ind=TRUE) # flag huge errors
    L1s <- length(L1)
    if(L1s>0){
      w[L1] <- 0  # set weights on those obs to zero
    }
    ...
    varyh <- n^0.5*nom^0.5/((den1*den2)^0.5) # Dan: p.417 Kafadar approach
  ...
  return(list(ymn,varyh,df,w,ybar,se))
